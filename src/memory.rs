
static TETRIS: &'static [u8] = &[  ];
static FONT: &'static [u8] = &[ 0xF0, 0x90, 0x90, 0x90, 0xF0, 0x20, 0x60, 0x20, 0x20, 0x70, 0xF0, 0x10, 0xF0, 0x80, 0xF0, 0xF0, 0x10, 0xF0, 0x10, 0xF0, 0x90, 0x90, 0xF0, 0x10, 0x10, 0xF0, 0x80, 0xF0, 0x10, 0xF0, 0xF0, 0x80, 0xF0, 0x90, 0xF0, 0xF0, 0x10, 0x20, 0x40, 0x40, 0xF0, 0x90, 0xF0, 0x90, 0xF0, 0xF0, 0x90, 0xF0, 0x10, 0xF0, 0xF0, 0x90, 0xF0, 0x90, 0x90, 0xE0, 0x90, 0xE0, 0x90, 0xE0, 0xF0, 0x80, 0x80, 0x80, 0xF0, 0xE0, 0x90, 0x90, 0x90, 0xE0, 0xF0, 0x80, 0xF0, 0x80, 0xF0, 0xF0, 0x80, 0xF0, 0x80, 0x80];

pub static mut CHIPMEM: &'static mut [ u8; 4096] = & mut [0;4096];
pub static mut REG: &'static mut [u8; 0x10] = & mut [0; 0x10]; //16 u8 registers
pub static mut KEYS: &'static mut [bool; 0x10] = & mut [false; 0x10]; //16 key inputs

pub static mut MEMPOINTER: usize = 0x00000200;
pub static mut STACKPOINTER: usize = 0x00000600;
pub static mut I: usize = 0;

pub static mut DELAYTIMER: u8 = 60;
pub static mut SOUNDTIMER: u8 = 0;

pub unsafe extern fn decrement_timers() {
	if DELAYTIMER > 0 {
		DELAYTIMER -= 1;
	}
	
	if SOUNDTIMER > 0 {
		SOUNDTIMER -= 1;
	}
}

pub unsafe fn return_subroutine()
{
	MEMPOINTER = (((CHIPMEM[STACKPOINTER + 1] as u16)  << 8) | (CHIPMEM[STACKPOINTER]) as u16) as usize;
	STACKPOINTER += 2;
}

pub unsafe fn call_subroutine(nnn: u16)
{
	STACKPOINTER -= 2;
	//write_mem_u16(STACKPOINTER, MEMPOINTER);
	CHIPMEM[STACKPOINTER + 1] = ((MEMPOINTER & 0xFF00) >> 8) as u8;
	CHIPMEM[STACKPOINTER] = (MEMPOINTER & 0x00FF) as u8;
	MEMPOINTER = nnn as usize;
}

pub unsafe fn jump(address: u16)
{
	MEMPOINTER = address as usize;
}

pub unsafe fn load_tetris() {
	let mut i: u16 = 0;
	for byte in TETRIS {
		CHIPMEM[MEMPOINTER + i as usize] = *byte;
		i += 1;
	}
}

pub unsafe fn load_font() {
	let mut i: u16 = 0;
	for byte in FONT {
		CHIPMEM[i as usize] = *byte;
		i += 1;
	}
}